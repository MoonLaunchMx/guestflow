...
export default function EventLayout({ children }: { children: React.ReactNode }) {
  ...
  // Items izquierda y derecha del botón central
  const LEFT_ITEMS  = NAV_ITEMS.slice(0, 2) // Invitados, Álbum
  const RIGHT_ITEMS = NAV_ITEMS.slice(2)    // Playlist, Config
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <aside className="hidden w-56 shrink-0 flex-col overflow-y-auto border-r border-[#e8e8e8] bg-[#f8f5f0] lg:flex">
          ...
          <nav className="flex-1 py-2">
            ...
            {/* WA Hub — sidebar desktop */}
            <div className="mx-3 mt-3 border-t border-[#e8e8e8] pt-3">
              <button
                onClick={goToHub}
                className="flex w-full items-center gap-2.5 rounded-lg bg-[#48C9B0]/10 px-3 py-2.5 text-left text-sm font-semibold text-[#0F6E56] transition hover:bg-[#48C9B0]/20"
              >
                {WA_ICON}
        ...
        {LEFT_ITEMS.map(item => (
          <button
            ...
            className={`flex flex-1 flex-col items-center justify-center gap-1 py-2.5 text-[10px] font-medium transition
              ${isActive(item.path) ? 'text-[#48C9B0]' : 'text-[#bbb] hover:text-[#888]'}`}
          >
